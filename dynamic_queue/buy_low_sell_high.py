...
class Buyer(object):
    ...
    def decide_whether_to_buy_or_sell(self, quote):
        ...
        print("Thinking about whether to buy or sell %s at %s"
              % (symbol, price))
        ...
        if len(self.history[symbol]) >= self.trend:
            ...
            print("Recent history of %s is %s" %
                  (symbol, self.history[symbol][-self.trend:]))
        else:
        ...
        print("Trending minimum/avg/max of %s is %s-%s-%s" %
              (symbol, price_low, price_avg, price_max))
        ...
        #    print("Value of %s is %s" % (symbol,
        #            float(self.holdings[symbol][0])*self.history[symbol][-1]))
        value = sum([self.holdings[symbol][0] * self.history[symbol][-1]
                     for symbol in self.holdings.keys()])
        print("Net worth is %s + %s = %s" %
              (self.cash, value, self.cash + value))

        if symbol not in self.holdings:
            if price < 1.01 * price_low:
                shares_to_buy = random.choice([10, 15, 20, 25, 30])
                print("I don't own any %s yet, and the price is below the \
                      trending minimum of %s so I'm buying %s shares." %
                ...
                if cost < self.cash:
                    ...
                    print("Unfortunately, I don't have enough\
                          cash at this time.")
        else:
            if price > self.holdings[symbol][1] and price > 0.99 * price_max:
                print("+++++++ Price of %s is higher than my holdings,\
                      so I'm going to sell!" % symbol)
                sale_value = self.holdings[symbol][0] * price
                print("Sale value is %s" % sale_value)
                print("Holdings value is %s" % self.holdings[symbol][2])
                print("Total net is %s" %
                      (sale_value - self.holdings[symbol][2]))
                self.cash += sale_value
                print("Cash is now %s" % self.cash)
                del self.holdings[symbol]

    def handle_pyamqplib_delivery(self, msg):
        self.handle(msg.delivery_info["channel"],
                    msg.delivery_info["delivery_tag"], msg.body)
